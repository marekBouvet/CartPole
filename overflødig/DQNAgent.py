...
class DQNAgent:
    ...
    def replay(self):
        ...
        states = np.vstack(minibatch[:, 0]).astype("float32")
        actions = minibatch[:,1]
        next_states = np.vstack(minibatch[:, 2]).astype("float32")
        rewards = np.array(minibatch[:,3])

        # Compute a new estimate for each Q-value. This uses the second half of Bellman's equation.
        estimate = rewards + self.gamma * np.amax(self.model.predict(next_states, verbose=0), axis=1)
